...
export default function Product_details() {
  ...
  useEffect(() => {
    if (typeof eventId === "string") {
      suKienDetailsService
        .getSuKienById(eventId)
        .then((data) => {
          if (data) {
            setSuKien(data);
          } else {
      ...
      <div style={{ zoom: "0.9" }}>
                ...
                <div>
                  ...
                  <div id="venue" className="">
                    <div className="d-flex gap-2 align-items-center">
                      ...
                      <span
                        className="venue-text fs-6 fw-bold"
                        style={{ color: " #2DC275" }}
                      >
                        {suKien.DiaDiem}
                      </span>
                  ...
                  <div className="ps-4">
                    <span
                      className="fw-bold"
                      style={{ fontSize: "14px", color: "rgb(196, 196, 207)" }}
                    ></span>
                  </div>
                </div>
                <div className="price">
                  {firstShow && (
                    <div className="d-flex align-items-baseline mt-3 mb-2 mb-1">
                      <span className="me-2 text-white fs-5 fw-bold">
                        Giá từ
                      </span>
                      <span
                        id="price-value"
                        className="fw-bold me-2"
                        style={{ fontSize: "25px", color: "rgb(45, 194, 117)" }}
                      >
                        {Intl.NumberFormat("vi-VN").format(
                          Math.min(
                            ...firstShow.loaiVes.map((ve) =>
                              parseFloat(ve.GiaVe)
                            )
                          )
                        )}{" "}
        ...
        <div className="event-info">
          <div className="container d-flex justify-content-between gap-4 p-0">
            <div
              className="left d-flex flex-column gap-4"
              style={{ width: "80%", borderRadius: " 10px" }}
            >
              <div
                className="p-3 bg-white rounded-3"
                style={{ boxShadow: "2px 2px 10px rgba(255, 255, 255, 0.5)" }}
              >
                <h2 className="">Giới thiệu</h2>
                <div
                  className={`event-details ${openDetail ? "expanded" : ""}`}
                >
                  <EventDetails htmlContent={suKien.ThongTinSuKien} />
                </div>
                <div
                  className="View-Details d-flex align-items-center gap-2 justify-content-center p-2 mt-3 border-0 w-100"
                  style={{ cursor: "pointer" }}
                  onClick={() => setopenDetail(!openDetail)}
                >
                  <i
                    className={`bi ${
                      openDetail ? "bi-chevron-up" : "bi-chevron-down"
                    }`}
                  ></i>
                      ...
                      <div className="p-0">
                        <div className="d-flex fs-6 gap-2 fw-bold align-items-center">
                          ...
                          <p className="m-0 fs-6">
                            <DisplayEventTime
                              start={suat.ThoiGianBatDau}
                              end={suat.ThoiGianKetThuc}
                            />
                          </p>
                        </div>
                      </div>
                      <Link
                        href={`/User/Book-Tickets?id_detail=${suKien?.IDSuKien}`}
                        ...
                        <div className="">
                          <ul className="list-unstyled text-uppercase m-0">
                            {suat.loaiVes.map((ve, index) => (
                              <li
                                key={ve.IDLoaiVe}
                                className={`ant-collapse-content-box fw-bold align-items-center d-flex justify-content-between p-3 ${
                                  index % 2 === 0 ? "bg-1" : "bg-2"
                                }`}
                              >
                                ...
                                <div>
                                  <p
                                    className={`fw-bold m-0 ${
                                      ve.SoLuongVe == 0 || ve.SoLuongVe == 0
                                        ? "text-gray"
                                        : "text"
                                    }`}
                                  >
                                    {parseInt(ve.GiaVe).toLocaleString("vi-VN")}{" "}
                                    VND
                                  </p>
                                  {ve.SoLuongVe == 0 && (
              ...
              <div className="bg-white rounded-3 ps-3 pe-3">
                <h5
                  className="pt-3 pb-3 fs-6 fw-semibold"
                  style={{ borderBottom: "1px solid rgb(235, 235, 240)" }}
                >
                  Ban tổ chức
                </h5>
